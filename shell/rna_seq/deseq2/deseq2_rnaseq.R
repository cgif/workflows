...
write.table(resOrdered, file = paste(results.dir, "results.tsv", sep="/"), sep = "\t", col.names = NA, quote = FALSE)
...
#pdf(file=paste(results.dir,"countmatrix_heatmap.pdf",sep="/"))
pheatmap(assay(vsd)[select,], cluster_rows=FALSE, show_rownames=FALSE,
cluster_cols=FALSE, annotation_col=df,filename=paste(results.dir,"countmatrix_heatmap.pdf",sep="/"))
...
#pdf(file=paste(results.dir,"sample_to_sample_distance_heatmap.pdf",sep="/"))
pheatmap(sampleDistMatrix,
clustering_distance_rows=sampleDists,
clustering_distance_cols=sampleDists,
col=colors,filename=paste(results.dir,"sample_to_sample_distance_heatmap.pdf",sep="/"))

#principal component plot of the samples

data <- plotPCA(vsd, intgroup=c("condition"), returnData=TRUE, ntop = 1000)
percentVar <- round(100 * attr(data, "percentVar"))
pdf(file = paste(results.dir,"pca_samples.pdf", sep="/" ))
