...
open (DATA, "/groupvol/cgi/resources/annotations/clinvar_variant_summary.2013-10-17.txt")||print "Can't open clinvar_variant_summary.2013-10-17.txt\n";
open (BED, ">/groupvol/cgi/resources/annotations/clinvar_variant_summary.2013-10-17.pathogenic.50bp.bed");
...
awk '{if (/#/) {next} if ($5 > 0) {print $4 "\t" $2 "\t" $3 "\t" $1 ":gain"} if ($5 < 0) {print $4 "\t" $2 "\t" $3 "\t" $1 ":loss"}}' decipher-hg19_13-11-26.txt > decipher-hg19_13-11-26.bed
