XHMM_plots <- function(PLOT_PATH, JOB_PREFICES, XHMM_PATH, JOB_TARGETS_TO_GENES=NULL, SAMPLE_FEATURES=NULL, SQ_THRESH=60, NUM_ADD_TARGS=2, PLOT_READ_DEPTHS=TRUE, PLOT_PC_CORRS=TRUE, PLOT_ALL_CNVS=TRUE, USE_XCNV_TO_PLOT=NULL, INCLUDE_PEDIGREE_SAMPLES=NULL, PLOT_ONLY_PNG=TRUE, LIMIT_MEMORY=FALSE) {
	...
	if (PLOT_ALL_CNVS) {
		...
		for (job in JOBS) {
			...
			for (cnvInd in 1:nrow(XCNV_CALLS)) {
				...
				chrom_start_stop = parseRegion(interval)
				MARK_INTERVALS=list()
