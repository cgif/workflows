...
system "cp blat_out_candidate_leftlane.fa blat_out_candidate_leftlane.0.fa";
system "cp blat_out_candidate_rightlane.fa blat_out_candidate_rightlane.0.fa";
...
if ($pair_end) {
  ...
  system "$compress_script blat_out_candidate_rightlane.fa $compress_ratio_thrd > compress_rightlane.names";
}
...
if ($pair_end) {
  system "$trinity_script --seqType fa --left bowtie_leftlane.fa --right bowtie_rightlane.fa --paired_fragment_length $paired_fragment_length --min_contig_length $min_contig_length --run_butterfly --output trinity_output --CPU $trinity_threads --bfly_opts '--stderr'";
}

else {
  system "$trinity_script --seqType fa --single bowtie_singlelane.fa --min_contig_length $min_contig_length --run_butterfly --output trinity_output --CPU $trinity_threads --bfly_opts '--stderr'";
}
...
system "cp Trinity.fasta Trinity.0.fasta";
system "cp human_contig.txt human_contig.0.txt";
system "cp clean_blastn.fa clean_blastn.0.fa";
...
for ($nth_iteration=2; $nth_iteration<=$iteration; $nth_iteration++) {
  ...
  if ($pair_end) {
    system "$trinity_script --seqType fa --left iteration_leftlane.fa --right iteration_rightlane.fa --paired_fragment_length $paired_fragment_length --min_contig_length $min_contig_length --run_butterfly --output trinity_output --CPU $trinity_threads --bfly_opts \"--stderr\"";
  }  
  else {
    system "$trinity_script --seqType fa --single iteration_singlelane.fa --min_contig_length $min_contig_length --run_butterfly --output trinity_output --CPU $trinity_threads --bfly_opts \"--stderr\"";
  }
