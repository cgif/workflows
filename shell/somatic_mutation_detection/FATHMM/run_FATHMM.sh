...
cat $TMPDIR/annotated.vcf | grep -v '#' | perl -e 'while(<>) {@data=split(/\;/,$_); $noncoding=1; $uncommon=1; foreach $key (@data) {if ($key =~ /^variant_classification=Frame_Shift_Del|^variant_classification=Frame_Shift_Ins|^variant_classification=De_novo_Start_OutOfFrame|^variant_classification=Start_Codon_SNP|^variant_classification=Splice_Site|^variant_classification=Nonsense_Mutation|^variant_classification=Nonstop_Mutation|^variant_classification=In_Frame_Del|^variant_classification=In_Frame_Ins|^variant_classification=Missense_Mutation|^variant_classification=Silent/) {$noncoding--} if ($key =~ /^1000Genome_AF=(.*)/) {$uncommon-- if $1 > 0.01} if ($key =~ /^ESP_MAF=(.*)/) {@AF = split(/,/,$1); $uncommon-- if $AF[2] > 1}} if ($noncoding == 1 && $uncommon == 1) {print}}' > $TMPDIR/noncoding.uncommon.annotated.vcf

#filter indels and put chromosome, coordinate, reference and mutation alleles into coma-separated file
cat $TMPDIR/noncoding.uncommon.annotated.vcf | grep -v '#' | perl -e 'while(<>) {@data=split(/\;/,$_); $f_snv=0; foreach $key (@data) {$f_snv++ if $key =~ /genome_change=.*del|genome_change=.*ins/} print unless $f_snv;}' | perl -e 'while(<>) {@data=split(/\t/,$_); $ref = $1 if $data[3] =~ /^(\w).*/; $mut = $1 if $data[4] =~ /^(\w).*/; print "$data[0],$data[1],$ref,$mut\n"}'  > $TMPDIR/noncoding.uncommon.snv.csv
...
cat $TMPDIR/noncoding.uncommon.snv.FATHMM_scores.txt | grep -vP '#|No Prediction Found' | awk '($5 > 0.5 && $6 == "ALL") {print}' | cut -f 1,2 > $TMPDIR/noncoding.uncommon.snv.FATHMM_scores.selected.txt
