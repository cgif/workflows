...
png(paste(metrics.file, ".", reads.aligned.ext, ".png", sep=""), width=width+230, height=500)

ggplot(reads.aligned.df,aes(names,reads,fill=status))+geom_bar(stat="identity")+scale_fill_hue(h.start=240,guide=guide_legend(title=NULL),labels=c("non-duplicate, uniquely mapped reads", "non-duplicate, non-uniquely mapped reads", "unmapped + mapped duplicate reads"))+ggtitle("UNIQUE AND ALIGNED READS")+scale_x_discrete("SAMPLE ID")+scale_y_continuous("READS, K",expand = c(0, 0),labels = comma)+theme(plot.title=element_text(face="bold",vjust=2),axis.title.x=element_text(vjust=-0.5),axis.text.x=element_text(angle=90,vjust=0.5),axis.title.y=element_text(vjust=1),panel.background=element_rect(fill="white"),panel.grid.major.x=element_line(colour="NA"),panel.grid.major.y=element_line(colour="light grey"))
...
ggplot(metrics,aes(rownames(metrics),MEAN_TARGET_COVERAGE))+geom_bar(stat="identity",fill="#DD8888")+ggtitle("MEAN COVERAGE FOR TARGETS with at least x2 coverage at one base")+scale_x_discrete("SAMPLE ID")+scale_y_continuous("COVERAGE, X",expand = c(0, 10))+theme(plot.title=element_text(face="bold",vjust=2),axis.title.x=element_text(vjust=-0.5),axis.text.x=element_text(angle=90,vjust=0.5),axis.title.y=element_text(vjust=1),panel.background=element_rect(fill="white"),panel.grid.major.x=element_line(colour="NA"),panel.grid.major.y=element_line(colour="light grey"))
...
ggplot(metrics,aes(rownames(metrics),FOLD_ENRICHMENT))+geom_bar(stat="identity",fill="#DD8888")+ggtitle("FOLD ENRICHMENT")+scale_x_discrete("SAMPLE ID")+scale_y_continuous("FOLDS, X",expand = c(0, 10))+theme(plot.title=element_text(face="bold",vjust=2),axis.title.x=element_text(vjust=-0.5),axis.text.x=element_text(angle=90,vjust=0.5),axis.title.y=element_text(vjust=1),panel.background=element_rect(fill="white"),panel.grid.major.x=element_line(colour="NA"),panel.grid.major.y=element_line(colour="light grey"))
...
coverage.df <- data.frame(names=factor(names,levels=rownames(metrics)), coverage=factor(coverage,levels=c("2","10","20","30")), targets.covered=targets.covered)

png(paste(metrics.file, ".", cumulative.coverage.ext, ".png", sep=""), width=width, height=500)

ggplot(coverage.df,aes(coverage,targets.covered,colour=names,group=names))+geom_point(shape=4,size=4)+geom_line(linetype=1,alpha=I(1/2))+guides(col = guide_legend(title=NULL,nrow = 25))+ggtitle("PERCENTAGE OF TARGET BASES ACHIEVING X COVERAGE OR GREATER")+scale_x_discrete("COVERAGE, X")+scale_y_continuous("BASES, %",expand = c(0, 0),lim=c(0,100))+theme(plot.title=element_text(face="bold",vjust=2),axis.title.x=element_text(vjust=-0.5),axis.title.y=element_text(vjust=1),panel.background=element_rect(fill="white"),panel.grid.major.x=element_line(colour="NA"),panel.grid.major.y=element_line(colour="light grey"))
